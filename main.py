...
with st.sidebar:
    amazon = Image.open(r'logofix.png')
...
if selected=="Dashboard":
    ...
    if cat_opt=="Category":
        ...
        with viz2:
            ...
            fig = go.Figure(go.Bar(
                                    ...
                                    marker_color="#e8962f"))
            fig.update_layout(title={
                                        'text': "Top 10 Categories with Highest Rating",
                                        'y':0.9,
                                        'x':0.5,
                                        'xanchor': 'center',
                                        'yanchor': 'top'})
        ...
        with viz1:
            ...
            fig.update_layout(title={
                                        'text': "Top 10 Sub-Categories",
                                        'y':0.9,
                                        'x':0.5,
                                        'xanchor': 'center',
                                        'yanchor': 'top'})
            fig.update_xaxes(title='Counts')
            fig.update_yaxes(title='Sub-Categories')
            #fig.update_xaxes(visible=False)
            fig.layout.xaxis.fixedrange = True
            fig.layout.yaxis.fixedrange = True
            st.plotly_chart(fig,use_container_width=True)
        with viz2:
            ...
            fig = go.Figure(go.Bar(
                                    ...
                                    marker_color="#e8962f"))
            fig.update_layout(title={
                                        'text': "Top 10 Sub-Categories with Highest Rating",
                                        'y':0.9,
                                        'x':0.5,
                                        'xanchor': 'center',
                                        'yanchor': 'top'})
    ...
    if method == "Collaborative Filtering":
        ...
        recommend_product_df = df[df['product_name'].isin(recommend_product)][['product_name', 'rating', 'rating_count', 'actual_price']].rename(columns={'product_name': 'Product Name', 'rating': 'Rating', 'rating_count': 'Numbers Reviews', 'actual_price': 'Price (₹)'}).groupby('Product Name').mean().sort_values('Rating', ascending=False)
        recommend_product_df['Price (IDR)'] = recommend_product_df['Price (₹)'] * 193
        st.dataframe(recommend_product_df, use_container_width=True)
        st.markdown("**Note: 1₹ = Rp193,04**")
        ...
        top_products = pd.DataFrame({'Product Name':[df.iloc[score[0]]['product_name'] for score in sim_scores[1:11]],
                                     ...
                                     'Rating':[df.iloc[score[0]]['rating'] for score in sim_scores[1:11]],
                                     'Numbers Reviews':[df.iloc[score[0]]['rating_count'] for score in sim_scores[1:11]],
                                     'Price (₹)':[df.iloc[score[0]]['actual_price'] for score in sim_scores[1:11]],
                                     }).groupby('Product Name').mean().round(2).sort_values('Similarity Score',ascending=False)
        
        top_products['Price (IDR)'] = top_products['Price (₹)']*193
